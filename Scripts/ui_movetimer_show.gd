...
func _process(_delta):
	...
	if len(activeLabels)>0:
		if GameManager.arePlayersAlive[0]==true:
			if players[0].moveTimer.is_stopped()==false and players[0].moveTimer.time_left<3.0:
			...
			if playerCount>=2:
				if players[1].moveTimer.is_stopped()==false and players[1].moveTimer.time_left<3.0:
			...
			if playerCount>=3:
				if players[2].moveTimer.is_stopped()==false and players[2].moveTimer.time_left<3.0:
			...
			if playerCount>=4:
				if players[3].moveTimer.is_stopped()==false and players[3].moveTimer.time_left<3.0:
